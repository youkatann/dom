...
const ROOMS = [
  { id: 'r3', title: 'Suite with terrace', slug: 'suite-terrace', hero: '025_DOM-HDR.jpg', stats: [{ label: 'Житлова площа', value: '43.8 м²' }, { label: 'Місткість', value: '2-4 особи' }] },
  { id: 'r4', title: 'Duplex suite with terrace', slug: 'duplex-suite-terrace', hero: '469_DOM-HDR.jpg', stats: [{ label: 'Житлова площа', value: '67,5 м²' }, { label: 'Місткість', value: '3-5 осіб' }] },
  { id: 'r5', title: 'Premium cottage with large terrace', slug: 'premium-cottage', hero: '669_DOM-HDR.jpg', stats: [{ label: 'Житлова площа', value: '62 м²' }, { label: 'Місткість', value: '2-4 осіб' }] },
]

// Мемоізована ліва колонка (hero)
const RoomHero = ({ room, hover, cursor, onClick, onMouseEnter, onMouseLeave, onMouseMove }) => (
  <div
  ...
  >
    {room?.hero ? (
      <img src={room.hero} alt={room.title} className="absolute inset-0 w-full h-full object-cover" />
    ) : (
    ...
    <div className="pointer-events-none absolute bottom-0 left-0 right-0 p-[16px] sm:p-[24px] md:p-[32px] text-neutral">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-[8px] sm:gap-[16px]">
        <h3 className="text-[32px] sm:text-[48px] md:text-[60px] lg:text-[40px] leading-[1] font-bold uppercase">{room.title}</h3>
      </div>
...
// Мемоізований елемент правої колонки (список кімнат)
const RoomsListItem = ({ room, active, onClick }) => (
  <li>
    <button
      onClick={onClick}
      className={`w-full text-left transition overflow-hidden cursor-pointer ${active ? 'bg-accent text-neutral' : 'bg-[rgb(149_149_149_/_0.1)] hover:bg-accent'}`}
    >
      <div className="grid grid-cols-[100px_1fr] sm:grid-cols-[112px_1fr] gap-[12px] sm:gap-[16px] p-[10px] sm:p-[12px] md:p-[14px]">
        <div className="relative w-[100px] sm:w-[112px] h-[70px] sm:h-[84px] overflow-hidden bg-zinc-200">
          {room.hero && <img src={room.hero} alt={room.title} className="absolute inset-0 h-full w-full object-cover" />}
        </div>
...
export default function Rooms() {
  ...
  const handleMouseMove = useCallback((e) => setCursor({ x: e.clientX, y: e.clientY }), [])
  const goToDetails = useCallback(() => selected && router.push(`/rooms/${selected.slug}`), [selected, router])
  const handleHoverEnter = useCallback(() => setHover(true), [])
  const handleHoverLeave = useCallback(() => setHover(false), [])

  return (
    <section id="Rooms" className="mt-[80px] px-[16px] sm:px-[24px] md:px-[40px] py-[40px] bg-neutral flex flex-col gap-[24px]">
